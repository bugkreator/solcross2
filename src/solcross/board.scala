...
class Board (val cells : Array[Array[Int]], val MovesSoFar: List[BoardMove])
{
  ...
  def getBestMoveList(): List[BoardMove] = {
    Board.setDepth(MovesSoFar.length)
  ...
  def getPossibleMoves(): List[BoardMove] = {
    if (MovesSoFar.length >= Board.maxDepth) List() else Board.allowableMoves.filter(
  ...
  override def toString() =
  {
    ...
    for (i <- 0 until Board.BoardSize)
    {
      ...
      {
        s.append(if ( (i==rowFrom) && (j==colFrom) ) "-" else  if ( (i==rowTo) && (j==colTo) ) "+" else ( cells(i)(j) match {
          case Board.CELL_OFF => " "
...
object Board {
  ...
  def setDepth(Depth: Int) : Unit = {
    if (Depth > currDepth) {
      println ("New Depth: " + Depth + ". n = " + n)
      currDepth = Depth
    }

  }

  val CELL_FREE: Int = 0 // TODO: Enum?
  val CELL_FULL: Int = 1
  val CELL_OFF: Int = -1
